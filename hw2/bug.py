...
X_train = torch.from_numpy(X_train).float().reshape(-1, 32, 32, 3)
X_test = torch.from_numpy(X_test).float().reshape(-1, 32, 32, 3)
...
def train(epoch):
    ...
    while (True):
        ...
        data = Variable(X_train[down:up, :])
        target = Variable(Y_train[down:up])
