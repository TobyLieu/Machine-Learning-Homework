...
for i in range(epoch):
    ...
    cel_loss_sum = 0
    for l in range(n):
        cel_grad_sum += (sigmoid(np.dot(x[l], w)) - y[l]) * x[l].reshape(784, 1)
        cel_loss_sum += -(y[l] * np.log(sigmoid(np.dot(x[l], w))) + (1 - y[l]) * np.log(1 - sigmoid(np.dot(x[l], w))))
        # print(cel_loss_sum)
    cel_grad = cel_grad_sum / n
    cel_loss = cel_loss_sum / n
    print(i, cel_loss)
    if(abs(cel_old_loss - cel_loss) < 1e-4):
        break
    w = w - lr * cel_grad
...
for l in range(test_data.shape[0]):
    if(sigmoid(np.dot(test_data[l], w)) - test_label[l] < 0.5):
        right += 1
